...
**CodeBuddy 是实际写代码的那个**：教程的定位已按真实工作流写——Termux 登录 → 在服务器上跑
CodeBuddy → 它改代码、跑构建和测试 → 人下需求、批方案、验收。

- 明确**CodeBuddy 装在服务器（那台 Debian 台式机）上，不是手机上**：顺序永远是
  先 `ssh home` 登录，再进仓库目录、开 tmux 会话、启动它。那台机器对外的角色就是"服务器"：
  代码、模型、CodeBuddy 都在它上面，手机只是登录进去的一块屏幕。
- 第 5 节新增"登录到服务器之后，用它干活"：改代码 + 构建 + 回归一整套要几分钟到几十分钟
  （本机编一次 CUDA 版本 3~4 分钟），所以放进 tmux，断开也无所谓，挂回 `tmux attach` 继续。
- 验收标准写死：看 `git diff`、测试、实测 t/s 与落盘字节，
  以及 `--temp 0` 正文 md5 是否与改动前**逐字一致**——"更快但输出变了"是 bug 不是收益。
- 继续上次会话的方式让读者用 `codebuddy --help` 自己确认，不写死具体 flag（各版本名字不同）。
