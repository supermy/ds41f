...
static void print_model_runtime(FILE *fp, const help_colors *c,
    ...
    opt(fp, c, "--host-offload-token-embd", "CUDA: keep token_embd in pinned host memory and read it over PCIe instead of device memory, freeing VRAM for the expert or context budget. Zero-cost where the embedding is gathered one row per token.");
    opt(fp, c, "--ram-resident-experts off|auto|NGB", "CUDA SSD streaming: when the routed expert weights fit in RAM, pin them in host memory once at startup and turn every staged read into a host-to-device copy. Auto (default) enables this only if the whole routed set fits next to a host-memory reserve; otherwise it falls back to a read-through host expert cache, whose budget NGB then caps instead.");
