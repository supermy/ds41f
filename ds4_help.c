...
static void print_model_runtime(FILE *fp, const help_colors *c,
    ...
    opt(fp, c, "--ram-resident-experts off|auto|NGB", "CUDA SSD streaming: when the routed expert weights fit in RAM, pin them in host memory once at startup and turn every staged read into a host-to-device copy. Auto (default) enables this only if the whole routed set fits next to a host-memory reserve; NGB caps the pool.");
