...
- 装不进内存时的读透式主机缓存：V4.1 Q2 解码 3.85 → 6.30 t/s，GLM 5.3 Flash 3.82 → 6.07 t/s
  （第 4 节与第 9 节）。
- 压低主机预留让 GLM 5.3 也全量常驻：6.07 → **7.20 t/s**（第 9 节）。
...
| 2026-09-19 | 覆盖「装不进内存」的模型 | 读透式主机专家缓存 | V4.1 Q2：3.85 → **6.30**；GLM 5.3：3.82 → **6.07** | 已实现 |
| 2026-09-19 | 把显存专家槽推到卡的极限 | 960 槽（1200+ 会 OOM） | V4 Flash IQ2XXS：13.55 → **15.19** | 已实现 |
| 2026-09-19 | 让 GLM 也全量常驻 | `DS4_RAM_RESIDENT_RESERVE_MB` 压低预留 | GLM 5.3：6.07 → **7.20** | 已实现 |
...
GLM 的两档都实测过：默认预留下它差 2.6 GiB 装不下，走读透缓存是 6.07 t/s
（落盘 98 GiB、命中率 70.2%）；把预留压到 6 GiB（`DS4_RAM_RESIDENT_RESERVE_MB=6144`）后
81.63 GiB 全量 pin 进主机内存，7.20 t/s，落盘 89 GiB、prefill 7.51 t/s。
从"上游基线"起算的四档完整数据（1.57 → 3.82 → 6.07 → 7.20，含每档落盘字节与复现命令）
见 [OPTIMIZATIONS_VS_UPSTREAM.md](OPTIMIZATIONS_VS_UPSTREAM.md)。
...
| GLM 5.3 Flash | 2.61 GB | 6.07（读透）/ **7.20（全常驻）** t/s | 15.8 / 18.8 GB/s | 11.0 t/s（**实测只到 7.20**，见下） |
...
|---|---|---|---|
| GLM 5.3 Flash | 81.63 GiB（+ 预留） | **本机就行**：把预留压到 6–8 GiB | 6.07 → **7.20 t/s**（实测） |
...
# GLM 5.3 Flash Q2：7.20 t/s（全常驻；不想动预留就去掉第二行，落回 6.07 t/s 的读透档）
DS4_GLM_MEMORY_GUARD=0 DS4_RAM_RESIDENT_RESERVE_MB=6144 ./ds4 --cuda -m <GLM53-Q2.gguf> \
      --ssd-streaming -c 2048 --nothink --temp 0 -n 128 -p "<prompt>"
```
