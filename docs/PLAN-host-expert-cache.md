...
> **状态：已实现（2026-09-19）。** 首次落盘 2026-09-18，按"保存计划、先不实施"冻结；
> 2026-09-19 解冻并落地。下面保留原始方案作为实现记录，文末列出与方案的偏差。
>
> 第 0 节（零代码对照实验）的结论是开工依据：**B2 相对 A +72%**，远超"≥20% 才动手"的判据。
> 实测结果、预算扫描与取舍见 `changelog.md` 与
> [OPTIMIZATION_HISTORY.md](OPTIMIZATION_HISTORY.md) 第 4 节。

## 实现后的偏差（相对本文档原方案）

1. **填充开关按"请求里的专家数"判定，不按 decode/prefill 入口**。原方案假设
   `begin_selected_load` 是解码、`prepare_selected_batch` 是 prefill；实测 DeepSeek 的
   decode 与 prefill 走的是同一个 `begin_selected_load`（slot 数分布：6 × 1240 次 = 解码，
   12 / 48 = prefill），而 `prepare_selected_batch` 在这里根本不被调用。改成
   `slot_count <= 8`（`DS4_HOST_EXPERT_CACHE_SLOTS` 可调）后语义才正确。
2. **默认预算取可用内存的 1/3（上限 32 GiB）**，不是全部可用内存。预算扫描显示
   16 GiB 已 6.23 t/s、32 GiB 6.30、78 GiB 反而 6.17；32 GiB 命中率 75.7%，
   78 GiB 只有 77.0%。多 pin 的内存换不到命中，只挤压页缓存。显式 `NGB` 时按用户给的值走。
3. **顺序回退路径 `cuda_model_copy_to_device_streamed` 未接缓存**（原方案第 2 节第 4 条）：
   它只在并行暂存起不来时才走，收益面很小。
4. 其余按原方案实现：三个 arena、CLOCK 淘汰、按偏移索引、预读只查不填、
   三处拆卸（cleanup / set_model_map / set_model_fd_for_map）。
