...
**Termux 登录 → 在服务器上跑 CodeBuddy → 它改代码、跑构建和测试 → 你看结果验收。**

那台 Debian 台式机在这里就是你的"服务器"：代码在它上面、模型在它上面、
CodeBuddy 也装在它上面。手机只是登录进去的一块屏幕，你人在哪里不重要。
...
**CodeBuddy 装在服务器上**，不是手机上。所以顺序永远是：先 SSH 登录，
再在那台机器上启动它。它是终端程序，正好放进前面说的 tmux 会话：

```sh
ssh home              # 1) 先登录到服务器
cd ~/ds41f            # 2) 进仓库目录（它要知道在哪干活）
tmux new -s work      # 3) 开会话
codebuddy             # 4) 在会话里启动，把需求交给它
# —— 手机锁屏、网络切换、SSH 断了 ——
tmux attach -t work   # 5) 回来，它还在原处等你
```

好处是它改代码、跑 `make`、跑测试这一整套可能要好几分钟到几十分钟
（本机编一次 CUDA 版本就要 3~4 分钟），你不需要保持连接——挂回会话就能接着看。

会话里的 slash 命令用 `/help` 查，提交相关的是 `/commit`。
想接着上次的对话继续，先看 `codebuddy --help` 里有没有继续/恢复的选项
（不同版本名字不一样，别照抄网上写的 flag）。
